...
export function GoalPage({ onBack, dark, goal, onGoalChange }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className={`rounded-2xl p-6 ${dark ? 'bg-white/5' : 'bg-white/20'}`} style={{ backdropFilter: 'blur(20px)' }}>
        <div className="text-center mb-6">
          <span className={`text-6xl font-bold ${dark ? 'text-[#4fc3f7]' : 'text-[#0288d1]'}`}>
            {tempGoal}
          </span>
          <span className={`text-xl ml-2 ${dark ? 'text-white/60' : 'text-white/80'}`}>
            {t('cups')}
          </span>
        </div>
      ...
      <motion.button
        ...
        className={`w-full mt-6 p-4 rounded-2xl font-bold ${
          dark ? 'bg-[#4fc3f7] text-white' : 'bg-[#0288d1] text-white'
        }`}
      >
        {t('save')}
      </motion.button>
